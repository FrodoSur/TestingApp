...
import com.myApp.tests.repository.FakeGitHubRepository
import com.myApp.tests.view.search.ScreenState
import com.myApp.tests.view.search.SearchViewModel
...
class SearchViewModelTest {
    ...
    @Test
    fun coroutines_TestReturnValue_NullTotalCount() {
        testCoroutineRule.runBlockingTest {
            val observer = Observer<ScreenState> {}
            val liveData = searchViewModel.subscribeToLiveData()
    ...
    @Test
    fun coroutines_TestReturnValue_NullSearchResult() {
        testCoroutineRule.runBlockingTest {
            val observer = Observer<ScreenState> {}
            val liveData = searchViewModel.subscribeToLiveData()

            Mockito.`when`(repository.searchGithubAsync(SEARCH_QUERY)).thenReturn(
                SearchResponse(10, null)
            )
